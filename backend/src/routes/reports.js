...
// A cell a spreadsheet would evaluate instead of display. Prefixing with an apostrophe keeps the
// original text visible while forcing Excel, Sheets and LibreOffice to treat it as a literal.
      ...
      db.all(
        ...
        (err, workEntries) => {
          ...
          const rows = workEntries.map((entry) => ({
            date: csvCell(entry.date),
            hours: entry.hours,
            description: csvCell(entry.description),
            created_at: csvCell(entry.created_at)
          }));
